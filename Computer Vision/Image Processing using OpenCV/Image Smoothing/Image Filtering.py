...
#Configuring settings for the filtering
k = 7
ddepth = -1
kernel = np.ones((k,k), np.float32)/(pow(k,2))

#Applying filtering to the noisy image yielded
filtered = cv2.filter2D(obj, ddepth, kernel)

#Displaying the image
cv2.imshow("Actual Image", img)
cv2.imshow("Object Image", obj)
cv2.imshow("Filtered Object Image", filtered)
